...
func CreateBranch(name string) error {
	...
	commit, err := utils.GetLastCommitHash(branch)
	if err != nil {
...
func CheckoutBranch(name string) error {
	...
	oldTreeCommit, err := utils.GetCommitTreeHash(oldBranch)
	if err != nil {
		return fmt.Errorf("failed to get last commit hash for branch %s: %w", oldBranch, err)
	}

	newTreeCommit, err := utils.GetCommitTreeHash(name)
	if err != nil {
		return fmt.Errorf("failed to get last commit hash for branch %s: %w", name, err)
	}
	if oldTreeCommit != newTreeCommit {
		newFiles, err := utils.GetFiles(newTreeCommit, "")
		if err != nil {
			return fmt.Errorf("failed to get new files for branch %s: %w", name, err)
		}
		oldFiles, err := utils.GetFiles(oldTreeCommit, "")
		if err != nil {
			return fmt.Errorf("failed to get old files for branch %s: %w", name, err)
		}
		...
		for path, newFile := range newFiles {
			...
			err := utils.WriteNewFiles(fl)
			if err != nil {
				return fmt.Errorf("failed to write file %s: %w", newFile.Path, err)
			}
		}
		...
		for path := range oldFiles {
			if _, exists := newFiles[path]; !exists {
				...
				if err != nil && !os.IsNotExist(err) {
					return fmt.Errorf("failed to delete removed file %s: %w", path, err)
				}
			}
		}

		// Rebuild the .kit/INDEX file
		f, err := os.OpenFile(".kit/INDEX", os.O_CREATE|os.O_WRONLY|os.O_TRUNC, 0644)
		if err != nil {
		...
		for _, file := range newFiles {
			if file.Mode == "100644" {
				...
				if _, err := f.WriteString(line); err != nil {
					return fmt.Errorf("failed to write line to INDEX: %w", err)
				}
			}
		}
	}

	if err := os.WriteFile(".kit/HEAD", []byte("ref: refs/heads/"+name), 0644); err != nil {
